...
class Game:
    ...
    def setup(self):
        ...
        go = Command(
            "go",
            " <direction> : se déplacer dans une direction cardinale (Up, E, Down, O)",
            Actions.go,
            1,
        )
        self.commands["go"] = go
        ...
        back = Command(
            "back",
            " : revenir dans la salle précédente",
            Actions.back,
            0,
            consumes_turn=False,
        )
        ...
        look = Command(
            "look",
            " : regarder les items présents dans la salle",
            Actions.look,
            0,
            consumes_turn=False,
        )
        ...
        take = Command(
            "take", " <item> : prendre un objet", Actions.take, 1, consumes_turn=True
        )
        self.commands["take"] = take
        drop = Command(
            "drop", " <item> : déposer un objet", Actions.drop, 1, consumes_turn=True
        )
        ...
        talk = Command(
            "talk", " : parler à un PNJ", Actions.talk, 1, consumes_turn=False
        )
        self.commands["talk"] = talk
        fight = Command(
            "fight", " : combattre un ennemi", Actions.fight, 1, consumes_turn=True
        )
        ...
        train = Command(
            "train",
            " : s'entraîner auprès d'un maître",
            Actions.train,
            0,
            consumes_turn=False,
        )
        ...
        upgrade = Command(
            "upgrade",
            " : améliorer une arme auprès d'un forgeron",
            Actions.upgrade,
            0,
            consumes_turn=False,
        )
        ...
        howling_cliffs = Room(
            "Howling Cliffs",
            "non loin des frontières du royaume située au nord-ouest. Les falaises hurlantes n'ont pas l'air peuplées du tout.",
        )
        self.rooms.append(howling_cliffs)
        self.dirtmouth = Room(
            "Dirtmouth",
            "dans un petit village presque abandonné, nommé Dirtmouth. Il y a quand même un peu de lumière et de vie cependant. (Une des maisons semble être une sorte de magasin mais il est actuellement fermé)",
        )
        self.rooms.append(self.dirtmouth)
        crystal_peak = Room(
            "Crystal Peak",
            "à une certaine hauteur, dans un lieu scintillant prénommé Mont Cristal. De nombreuses mines ont été creusées dans la falaise pour obtenir les précieux cristaux roses.",
        )
        self.rooms.append(crystal_peak)
        greenpath = Room(
            "Greenpath",
            "à Vertchemin, un lieu dont la végétation luxuriante et les insectes recouverts de feuillage en font un endroit splendide. On trouve également des flaques d'acides brûlant et une épaisse prolifération rendant la navigation difficile.",
        )
        self.rooms.append(greenpath)
        self.forgotten_crossroads = Room(
            "Forgotten Crossroads",
            "au beau milieu des Routes Oubliées, situé sous le village de Dirtmouth. Elles permettaient autrefois de rejoindre de nombreuses autres régions du royaume.",
        )
        self.rooms.append(self.forgotten_crossroads)
        temple_black_egg = Room(
            "Temple of the Black Egg",
            "en face du Temple de l'Œuf Noir, on peut y entendre que quelque chose crie depuis l'intérieur et qu'un évènement risque de déterminer l'avenir du royaume.",
        )
        self.rooms.append(temple_black_egg)
        blue_lake = Room(
            "Blue Lake",
            "en face d'un lac bleu. Celui ci semble inoffensif et reposant à un tel point que l'on souhaiterait rester en face jusqu'à l'écroulement d'Hallownest.",
        )
        self.rooms.append(blue_lake)
        fog_canyon = Room(
            "Fog Canyon",
            "dans Brumes Canyon, un endroit effervescent et gélatineux de part les méduses qui y habitent. Cependant, gare à vous car les méduses peuvent exploser au contact d'une paroi tout comme leur nid. Une énorme structure s'y trouve.",
        )
        self.rooms.append(fog_canyon)
        fungal_wastes = Room(
            "Fungal Wastes",
            "dans la Caverne Nocive qui est une zone humide et recouverte de champignons de toutes les sortes. En plus de cela, des flaques d'acide caustiques sont abondantes dans toute la région.",
        )
        ...
        deepnest = Room(
            "Deepnest",
            "dans le Nid Profond. Son environnement lugubre et sombre, jonché d'épaisses couches de toile d'araignées, peut en faire fuir beaucoup. Malgré tout, des traces d'anciennes civilisations sont présentes, ce qui en fait un lieu riche en histoire.",
            dark=True,
        )
        ...
        howling_cliffs.exits = {
            "Up": None,
            "E": self.dirtmouth,
            "Down": greenpath,
            "O": None,
        }
        self.dirtmouth.exits = {
            "Up": None,
            "E": crystal_peak,
        ...
        self.forgotten_crossroads.exits = {
            "Up": self.dirtmouth,
            "E": Door(temple_black_egg, locked=True),
            "Down": fungal_wastes,
            "O": greenpath,
        }
        temple_black_egg.exits = {
            "Up": None,
            "E": blue_lake,
            "Down": None,
            "O": self.forgotten_crossroads,
        }
        blue_lake.exits = {
            "Up": crystal_peak,
            "E": None,
        ...
        fungal_wastes.exits = {
            "Up": self.forgotten_crossroads,
            "E": Door(city_of_tears, locked=True, key="blason de la ville"),
            "Down": mantis_village,
            "O": fog_canyon,
        }
        ...
        journal_vagabond_1 = Item(
            "journal vagabond 1",
            "Ces journaux nous offrent un aperçu intéressant des esprits et des cœurs de ceux qui ont vécu avant nous.",
            value=500,
        )
        journal_vagabond_2 = Item(
            "journal vagabond 2",
            "Ces journaux nous offrent un aperçu intéressant des esprits et des cœurs de ceux qui ont vécu avant nous.",
            value=500,
        )
        journal_vagabond_3 = Item(
            "journal vagabond 3",
            "Ces journaux nous offrent un aperçu intéressant des esprits et des cœurs de ceux qui ont vécu avant nous.",
            value=500,
        )
        lanterne = Item(
            "lanterne",
            "Une lanterne en cristal qui éclaire les cavernes plongées de Deepnest afin que les voyageurs puissent retrouver leur chemin.",
            value=2000,
        )
        self.items[lanterne.name] = lanterne
        minerai_pale_1 = Item(
            "minerai pale 1",
            "Un minerai rare et pale, prié par ceux qui fabriquent des armes.",
            value=0,
        )
        minerai_pale_2 = Item(
            "minerai pale 2",
        ...
        cle_marchand = Item(
            "cle du marchand",
            "Une clé de cuivre qui ouvre la boutique du Marchand à Dirtmouth.",
            value=1,
        )
        sceau_hallownest_1 = Item(
            "sceau hallownest 1",
            "Ces sceaux ornés étaient les symboles officiels du Roi et de ses Chevaliers, et étaient précieux de ceux qui les portaient.",
            value=1000,
        )
        sceau_hallownest_2 = Item(
            "sceau hallownest 2",
            "Ces sceaux ornés étaient les symboles officiels du Roi et de ses Chevaliers, et étaient précieux de ceux qui les portaient.",
            value=1000,
        )
        sceau_hallownest_3 = Item(
            "sceau hallownest 3",
            "Ces sceaux ornés étaient les symboles officiels du Roi et de ses Chevaliers, et étaient précieux de ceux qui les portaient.",
            value=1000,
        )
        trampass = Item(
            "trampass",
            "Un laissez-passer ancien qui permet d'utiliser les tramways du royaume.",
            value=0,
        )
        idole_roi_1 = Item(
            "idole du roi 1",
            "Une icône du roi de Hallownest, vénéré à la fois comme dieu et comme souverain. Fabriqués dans un matériau blanc mystérieux, ils sont rares et très précieux.",
            value=1500,
        )
        idole_roi_2 = Item(
            "idole du roi 2",
        ...
        blason_ville = Item(
            "blason de la ville",
            "dalle de pierre arborant le blason de la capitale d'Hallownest.",
            value=1000,
        )
        self.items[blason_ville.name] = blason_ville
        masque_erudite = Item(
            "masque de l'érudite",
            "Un masque ancien qui augmente votre sagesse.",
            value=0,
        )
        aiguillon = Item(
            "aiguillon",
            "Un aiguillon simple mais efficace pour combattre les ennemis.",
            value=0,
        )
        oeuf_arcanique = Item(
            "oeuf arcanique",
            "Cela semble être un simple oeuf, mais c’est en réalité une relique précieuse d’avant la naissance de Hallownest !",
            value=3999,
        )
        ...
        self.forgotten_crossroads.inventory[sceau_hallownest_2.name] = (
            sceau_hallownest_2
        )
        ...
        sly = Character(
            "Sly",
            "Un insecte replié sur lui-même, marmonnant des paroles incohérentes.",
            self.forgotten_crossroads,
            ["…hein ?", "Laisse-moi tranquille…", "Je ne me souviens de rien…"],
            merchant=False,
            stock={},
        )
        ...
        lemm = Character(
            "Lemm",
            "Un chercheur de reliques passionné par l'histoire du royaume.",
            city_of_tears,
            [
                "Je me présente en tant que Lemm, chercheurs de reliques d'Hallownest. Il existe une histoire oubliée, cachée dans les antiquités de ce royaume. Cependant, rares sont ceux qui sont disposés à s'y intéresser. Certains viennent juste pour dénicher des trésors dans les fissures et dans les ruines",
                "Si tu trouves une sorte de journal, celui ci offre un aperçu intéressant dans l'esprit et le coeur de ceux qui ont vécu avant nous. Je pourrais vous l'acheter contre quelques Geo",
                "Les sceaux ornés étaient les symboles officiels du roi et de ses chevaliers, et chéris par ceux qui les portaient. Je vous en offrirai une modeste somme en échange.",
                ...
                "Il existe dans les profondeurs du royaume un oeuf qui parait simple. Mais c'est en fait, une précieuse relique qui remonte bien avant l'existence d'Hallownest ! Je vous en donnerai une petite fortune. Vendez-le-moi s'il vous plaît.",
            ],
            merchant=True,
        )
        ...
        forgeron = Character(
            "Forgeron",
            "Un forgeron en quête à la forge de l'aiguillon pur.",
            fungal_wastes,
            [
                "Mon aiguille est émoussée... tout comme ce royaume",
                "Apporte-moi 1500 Geo et un minerai pale, et je rendrai ton aiguillon digne d'un véritable guerrier.",
            ],
            blacksmith=True,
            upgrade_cost=1500,
        )
        ...
        elderbug = Character(
            "Elderbug",
            "Un des rares résident de Dirtmouth, il a l'air vieux et sage.",
            self.dirtmouth,
            [
                "J’ai bien peur qu’il ne reste plus que moi pour t’accueillir. Notre ville est devenue très silencieuse au fil du temps. Tous les autres résidents ont disparu.",
                "Je suis Elderbug. Si tu as besoin d’aide, n’hésite pas à me parler.",
                "Si tu cherches un marchand, tu le trouveras souvent aux Routes Oubliées ou à Dirtmouth. Il s’appelle Sly.",
            ],
            trainer=False,
        )
        ...
        monomon = Character(
            "Monomon l'érudite",
            "Soudain, un air très familier frappe Quirrel lorsqu'il aperçoit la Rêveuse face à lui.",
            fog_canyon,
            [
                "…Quirrel. Ton esprit n'est plus lié aux chaînes d'autrefois, et pourtant tu es revenu jusqu'à moi.",
        ...
        sheo = Character(
            "Sheo",
            "Un insecte robuste muni d'un pinceau. Sa technique semble être celle d'une entaille implacable.",
            deepnest,
            [
                "Oh ! Un Visiteur ? Voilà qui rompt la monotonie.",
                "Le combat est un art. Chaque mouvement doit être peint avec intention.",
                "Manie ton aiguillon comme un pinceau.",
            ],
            trainer=True,
            training_cost=1000,
        )
        ...
        oro = Character(
            "Oro",
            "Un insecte robuste muni d'un aiguillon énorme. Sa technique semble être celle d'une entaille rapide comme l'éclair.",
            crystal_peak,
            [
                "Ne perds pas de temps. L'hésitation est une faiblesse.",
                "La vitesse est la clé de la survie.",
                "Si tu veux apprendre, je te montrerais mon mouvement, la Coupe de Dash.",
            ],
            trainer=True,
            training_cost=3000,
        )
        ...
        mato = Character(
            ...
            [
                ...
                "ma technique, le Cyclone Slash, balaie tout sur son passage.",
            ],
            trainer=True,
            training_cost=5000,
        )
        ...
        self.zote = Character(
            "Zote",
            "L'incroyable et redoutable Zote se dresse devant vous ! Rare sont les créatures aussi faible que lui.",
            self.dirtmouth,
            [
        ...
        hollow_knight = Character(
            "Hollow Knight",
            "Il a été choisi il y a fort longtemps pour sceller la Radiance… Ce vaisseau était autrefois fils du roi et de la reine d'Hallownest.",
            temple_black_egg,
            ["..."],
            level=14,
            hostile=True,
        ...
        uumuu = Character(
            "Uumuu",
            "Une énorme méduse infecté par la Radiance. Il semble être le défenseur de la chambre de stase de Monomon.",
            fog_canyon,
            ["..."],
            level=10,
            hostile=True,
        ...
        soul_master = Character(
            "Le Maitre de l'ame",
            "Chef du Sanctuaire de l'âme dans la cité des larmes, il menait des expériences sur l'Ame pour trouver un autre moyen de repousser l'infection de la Radiance. Cela a échoué à priori…",
            city_of_tears,
            ["a faire"],
            level=12,
            hostile=True,
        ...
        hornet = Character(
            ...
            greenpath,
            [
                "Recule. Ce territoire n'est pas le tien.",
                ...
                "Si tu fais un pas de plus, je t'arrêterai moi-même.",
            ],
            level=3,
            hostile=True,
            is_boss=True,
        ...
        mantis_lord = Character(
            "Dames Mantes",
            "Trois sœurs Mante qui siègent chacune sur trois trônes. Leur coordination est redoutable et en font un ennemi coriace.",
            mantis_village,
            [
                "Étranger… tu as foulé un sol que peu d’âmes osent approcher.",
        ...
        crystal_guardian = Character(
            "Gardien de cristal",
            "Un ancien protecteur des mines de cristal, maintenant corrompu par l'infection.",
            crystal_peak,
            ["..."],
            level=2,
            hostile=True,
        ...
        false_knight = Character(
            "Faux Chevalier",
            "Un ver rendu fou par une force étrange. Il vit dans une carapace cuirassée qu’il a volé.",
            self.forgotten_crossroads,
            ["..."],
            level=1,
            hostile=True,
        ...
        mawlek = Character(
            "Mawlek Maussade",
            "Une bête féroce, mais extrêmement sociale. Elle devient agressive si elle ne peut pas s'amuser avec ceux de son espèce.",
            howling_cliffs,
            ["..."],
            level=4,
            hostile=True,
        ...
        nosk = Character(
            "Nosk",
            "Un prédateur métamorphe qui imite la forme de ses proies pour les attirer dans des embuscades.",
            deepnest,
            ["..."],
            level=9,
            hostile=True,
        ...
        elder_hu = Character(
            "Hu L'Ancien",
            "Un rêve persistant, appartenant à un guerrier mort. Il a voyagé partout dans ce monde et s’est occupé de ceux affectés par ce fléau.",
            fungal_wastes,
            ["..."],
            level=7,
            hostile=True,
    ...
    def _setup_quests(self):
        ...
        quest_oeuf = Quest(
            ...
            objectives=["prendre oeuf arcanique", "parler à Lemm"],
    ...
    def unlock_temple_black_egg(self):
        for room in self.rooms:
            for direction, exit_room in room.exits.items():
                if (
                    isinstance(exit_room, Door)
                    and exit_room.destination.name == "Temple of the Black Egg"
                ):
